...
public class Emitter implements Visitor {
  ...
  public void visit(AssignStmt x) {
    ...
    if (x.lAST instanceof VarExpr) {
      ...
      //                         emitFSTORE()
      //
      if(D.isGlobal()) emitStaticVariableReference(V.Ident, T, true);
      else{ // local variable
        if(T.Tequal(StdEnvironment.intType) || T.Tequal(StdEnvironment.boolType)){ // int
          if(D.index == -1) D.index = frame.getNewLocalVarIndex();
          emitISTORE(D.index);
        }
        else if(T.Tequal(StdEnvironment.floatType)){
          if(D.index == -1) D.index = frame.getNewLocalVarIndex();
  ...
  public void visit(IfStmt x) {
    ...
    emit(JVM.IFEQ + " Label" + Integer.toString(L1)); // ifeq L1
    x.thenAST.accept(this);
    //TBD: your code goes here...
    if(x.elseAST != null) emit(JVM.GOTO + " Label" + Integer.toString(L2)); // goto L2
    emitLabel(L1);
    if(x.elseAST != null) {
      x.elseAST.accept(this);
    }
    //TBD: your code goes here...
    if(x.elseAST != null) emitLabel(L2);
  }
  ...
  public void visit(WhileStmt x) {
    ...
    emitLabel(L1);
    x.eAST.accept(this);
    emit(JVM.IFEQ + " Label" + Integer.toString(L2)); // ifeq L2
    x.stmtAST.accept(this);
    emit(JVM.GOTO + " Label" + Integer.toString(L1)); // goto L1
    emitLabel(L2);
  }
  ...
  public void visit(VarExpr x) {
    ...
    //TBD: your code goes here...
    if(D.isGlobal()){
      emitStaticVariableReference(x.Ident, T, true);
    }
    else{
  ...
  public void visit(BinaryExpr x) {
    ...
    if(Op.equals("&&")) {
      ...
      emit(JVM.IFEQ + " Label" + Integer.toString(L1)); // ifeq L1
      emit("iconst_1");
      emit(JVM.GOTO + " Label" + Integer.toString(L2)); // goto L2
      emitLabel(L1);
      emit("iconst_0");
      emitLabel(L2);
      return;
    ...
    if(Op.equals("||")) {
      ...
      int L2 = frame.getNewLabel();
      x.lAST.accept(this);
      emit(JVM.IFNE + " Label" + Integer.toString(L1)); // ifne L1
      x.rAST.accept(this);
      emit(JVM.IFNE + " Label" + Integer.toString(L1)); // ifne L1
      emit("iconst_1");
      emit(JVM.GOTO + " Label" + Integer.toString(L2)); // goto L2
    ...
    //TBD:
    if(Op.equals("+")){
      if(x.oAST.type.Tequal(StdEnvironment.intType)) emit(JVM.IADD);
      else emit(JVM.FADD);
      return;
    }
    if(Op.equals("-")){
      if(x.oAST.type.Tequal(StdEnvironment.intType)) emit(JVM.ISUB);
      else emit(JVM.FSUB);
      return;
    }
    if(Op.equals("*")){
      if(x.oAST.type.Tequal(StdEnvironment.intType)) emit(JVM.IMUL);
      else emit(JVM.FMUL);
      return;
    }
    if(Op.equals("/")){
      if(x.oAST.type.Tequal(StdEnvironment.intType)) emit(JVM.IDIV);
      else emit(JVM.FDIV);
      return;
    }
    if(Op.equals(">")){
      ...
      if(x.oAST.type.Tequal(StdEnvironment.intType))
        emit(JVM.IF_ICMPGT + " Label" + Integer.toString(L1)); // if_icmpgt L1
      else emit(JVM.IFGT + " Label" + Integer.toString(L1)); // ifgt L1
      emit("iconst_0");
      emit(JVM.GOTO + " Label" + Integer.toString(L2)); // goto L2
      emitLabel(L1);
      emit("iconst_1");
    ...
    if(Op.equals(">=")){
      ...
      int L2 = frame.getNewLabel();
      if(x.oAST.type.Tequal(StdEnvironment.intType))
        emit(JVM.IF_ICMPGE + " Label" + Integer.toString(L1)); // if_icmpge L1
      else emit(JVM.IFGE + " Label" + Integer.toString(L1)); // ifge L1
      emit("iconst_0");
      emit(JVM.GOTO + " Label" + Integer.toString(L2)); // goto L2
      emitLabel(L1);
      emit("iconst_1");
    ...
    if(Op.equals("<")){
      ...
      int L2 = frame.getNewLabel();
      if(x.oAST.type.Tequal(StdEnvironment.intType))
        emit(JVM.IF_ICMPLT + " Label" + Integer.toString(L1)); // if_icmplt L1
      else emit(JVM.IFLT + " Label" + Integer.toString(L1)); // iflt L1
      emit("iconst_0");
      emit(JVM.GOTO + " Label" + Integer.toString(L2)); // goto L2
      emitLabel(L1);
      emit("iconst_1");
    ...
    if(Op.equals("<=")){
      ...
      int L2 = frame.getNewLabel();
      if(x.oAST.type.Tequal(StdEnvironment.intType))
        emit(JVM.IF_ICMPLE + " Label" + Integer.toString(L1)); // if_icmple L1
      else emit(JVM.IFLE + " Label" + Integer.toString(L1)); // ifle L1
      emit("iconst_0");
      emit(JVM.GOTO + " Label" + Integer.toString(L2)); // goto L2
      emitLabel(L1);
      emit("iconst_1");
    ...
    if(Op.equals("==")){
      ...
      int L2 = frame.getNewLabel();
      if(x.oAST.type.Tequal(StdEnvironment.intType))
        emit(JVM.IF_ICMPEQ + " Label" + Integer.toString(L1)); // if_icmpeq L1
      else emit(JVM.IFEQ + " Label" + Integer.toString(L1)); // ifeq L1
      emit("iconst_0");
      emit(JVM.GOTO + " Label" + Integer.toString(L2)); // goto L2
      emitLabel(L1);
      emit("iconst_1");
    ...
    if(Op.equals("!=")){
      ...
      int L2 = frame.getNewLabel();
      if(x.oAST.type.Tequal(StdEnvironment.intType))
        emit(JVM.IF_ICMPNE + " Label" + Integer.toString(L1)); // if_icmpne L1
      else emit(JVM.IFNE + " Label" + Integer.toString(L1)); // ifne L1
      emit("iconst_0");
      emit(JVM.GOTO + " Label" + Integer.toString(L2)); // goto L2
      emitLabel(L1);
      emit("iconst_1");
