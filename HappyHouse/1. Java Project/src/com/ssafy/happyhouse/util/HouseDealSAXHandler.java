...
public class HouseDealSAXHandler extends DefaultHandler {
	...
	/**아파트 거래 정보를 담는다 */
	private List<HouseDeal> houses = new LinkedList<HouseDeal>();
	/**파상힌 아파트 거래 정보*/
	...
	public void endElement(String uri, String localName, String qName){
		if(qName.equals("지역코드")) { // 법정 동코드
			house.setCode(Integer.parseInt(temp));
		}else if(qName.equals("연립다세대")) { // 아파트 이름
			house.setAptName(temp.trim());
		}else if(qName.equals("법정동")) { // 법정 동명
			house.setDong(temp.trim());
		}else if(qName.equals("거래금액")) { // 거래 금액
			house.setDealAmount(temp);
		}else if(qName.equals("건축년도")) { // 건축연도
			house.setBuildYear(Integer.parseInt(temp));
		}else if(qName.equals("년")) { // 거래 연도
			house.setDealYear(Integer.parseInt(temp));
		}else if(qName.equals("월")) { // 거래 월
			house.setDealMonth(Integer.parseInt(temp));
		}else if(qName.equals("일")) { // 거래 일
			house.setDealDay(Integer.parseInt(temp));
		}else if(qName.equals("전용면적")) { // 전용 면적
			house.setArea(Double.parseDouble(temp));
		}else if(qName.equals("지번")) { // 지번
			house.setJibun(temp);
		}else if(qName.equals("층")) { // 층
