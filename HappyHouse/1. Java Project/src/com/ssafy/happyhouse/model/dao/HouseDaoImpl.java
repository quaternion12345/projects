	...
	public HouseDeal search(int no) {
		...
		for (HouseDeal houseDeal : search) {
			if(houseDeal.getNo() == no) {
				String str = houseDeal.getDong() + houseDeal.getAptName();
				if(houseInfo.containsKey(str)) {
					HouseInfo Info = houseInfo.get(str);
					houseDeal.setBuildYear(Info.getBuildYear());
					houseDeal.setCode(Info.getCode());
					houseDeal.setJibun(Info.getJibun());
					houseDeal.setImg(Info.getImg());
				}
				return houseDeal;
			}
		}
