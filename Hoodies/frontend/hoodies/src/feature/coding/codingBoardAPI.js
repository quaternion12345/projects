...
export const createArticle = async (title, content) => {
    ...
    const formData = {'title': title, 'writer': writer, 'content': content, 'type': 3 }
    try {
...
export const fetchArticles = async (page) => {
    try {
        const response = await axios1.get(API_URL + `board/3?page=${page}&size=20`, {headers: {
            'accessToken': localStorage.getItem('token')
        }})
        return response.data
    } catch (err) {
...
export const createComment = async (articleId, content) => {
    const writer = localStorage.getItem('nickname')
    const formData = {'content': content, 'writer': writer, type: 3}
    try {
...
export const fetchSearch = async (option, keyword, page) => {
    try {
        const response = await axios.get(API_URL + `board/3/search?option=${option}&keyword=${keyword}&page=${page}&size=20`, {headers: {
            'accessToken': localStorage.getItem('token')
        }})
        return response.data
    } catch (err) {
